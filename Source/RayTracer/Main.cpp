...
int main(int argc, char**)
{
	...
	for (int i = 0; i < 50; i++)
	{
		std::shared_ptr<Material> material = (int(random(0, 2)) == 1) ? std::dynamic_pointer_cast<Material>(lambertian) : std::dynamic_pointer_cast<Material>(metal);
		auto sphere = std::make_unique<Sphere>(random({ -5, -5, -15 }, { 5, 5, -5 }), random01() * 3, material);
		scene.AddObject(std::move(sphere));
	}
		
	canvas.Clear({ 0, 0, 0, 1 });
	scene.Render(canvas, 25);
	canvas.Update();
